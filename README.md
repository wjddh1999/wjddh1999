# 권정오

### Unity Client / Gameplay Programmer

Unity와 C#을 기반으로 플레이어가 직접 체감하는 **조작, 전투, 상호작용, UI 흐름**을 구현합니다.

기능의 책임을 분리하고, 여러 시스템이 안정적으로 연결되는 게임플레이 구조를 만드는 데 집중합니다.

[**R.I.P Code Showcase**](https://github.com/wjddh1999/RIP-Showcase) · [**Playable Builds**](https://wjddh1998.itch.io/) · [**Portfolio**](https://app.notion.com/p/3a2a971ce6a28101bd9cd8fd9bd4a815?pvs=204)

---

## Featured Project

### R.I.P — Rusty Iron Project

Unity 6 기반 3D 메카 액션 프로젝트입니다. 고속 이동, 전투 파이프라인, 하드 록온과 Photon Fusion 2 기반 PvP 흐름을 구현하고 있습니다.

- CharacterController 기반 지상·공중 기동
- Quick Boost / Assault Boost와 에너지 관리
- Projectile · Shield · Armor · ACS 전투 처리
- Hard Lock 후보 탐색과 Lag Compensation
- State Authority 기반 데미지·승패 판정

[**Repository & Code Samples →**](https://github.com/wjddh1999/RIP-Showcase)

[**Windows Build on itch.io →**](https://wjddh1998.itch.io/rusty-iron-project)

## Other Work

### ReadyToDrive — Manual Transmission Driving Simulator

Standard Assets 차량 제어를 기반으로 클러치·기어 선택·RPM 조건을 연결한 수동 변속 규칙을 구현한 개인 프로젝트입니다.

[**ReadyToDrive Code Showcase →**](https://github.com/wjddh1999/ReadyToDrive-Showcase)

## Focus

- **Gameplay Systems** — Movement · Combat · Camera · Targeting
- **Interaction & UI** — 상태 기반 상호작용과 플레이어 피드백
- **Client Architecture** — 책임 분리와 데이터 중심 구조

## Tech Stack

![Unity](https://img.shields.io/badge/Unity-6000.4-000000?logo=unity&logoColor=white)
![C#](https://img.shields.io/badge/C%23-512BD4?logo=csharp&logoColor=white)
![Photon Fusion 2](https://img.shields.io/badge/Photon-Fusion%202-004480)
![Unity Gaming Services](https://img.shields.io/badge/Unity-Gaming%20Services-000000?logo=unity&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?logo=git&logoColor=white)

## Links

- [Portfolio](https://app.notion.com/p/3a2a971ce6a28101bd9cd8fd9bd4a815?pvs=204)
- [itch.io](https://wjddh1998.itch.io/)
